...
{
    public class IngredientService : DataService<IngredientDto>
    {
        public IngredientService(IDbContextFactory<ApplicationDbContext> contextFactory, ILoggerFactory logFactory)
            : base(contextFactory, logFactory)
        {
            Repository = new IngredientDataRepository(_contextFactory.CreateDbContext(), _log);
        }
    }
}